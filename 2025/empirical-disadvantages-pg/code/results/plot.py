...
def create_strip_with_heatbar(model_name):
    ...
    for i, alpha in enumerate(MATCHING_ALPHA_VALUES):
        ...
        print(f"Alpha {alpha}: Max reward = {subset['Reward'].max():.1f}, Episodes = {len(subset)}")
    ...
    # Update layout
    fig.update_layout(
        title=f"{model_name} – Strip Plot (Matching α_π = α_V)",
        xaxis_title="Episode",
        yaxis=dict(
            title="α (π = V)",
            tickmode='array',
...
# Generate figures
print("Creating PolicyGradient figure...")
fig_pg = create_strip_with_heatbar('PolicyGradient')

print("Creating PPOClip figure...")
